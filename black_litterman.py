...
if __name__ == "__main__":
    ...
    print("=== Black-Litterman Quick Test ===\n")

    # --- Simulated inputs (replace with real data from teammates) ---
    TICKERS = ["AAPL", "MSFT", "GOOGL", "AMZN", "JPM", "JNJ", "XOM"]
    prices = yf.download(TICKERS, start="2022-01-01", end="2024-12-31",
                         auto_adjust=True)["Close"]
    prices.dropna(inplace=True)

    # Simulated ML predictions (your teammate's output format)
    ml_predictions = pd.DataFrame({
        "ticker":     ["AAPL", "MSFT", "AMZN"],
        "return":     [0.14,   0.18,   0.20  ],   # 14%, 18%, 20% annualized
        "confidence": [0.7,    0.8,    0.6   ],   # model confidence
    })

    # Simulated questionnaire answers
    answers = {"q1": 4, "q2": 3, "q3": 4, "q4": 3, "q5": 4}
    risk_score = score_risk_questionnaire(answers)
    print(f"Risk Score: {risk_score}/10 — {RISK_LABELS[risk_score]}")
